...
message("=== Loading Data with PC2 Covariate ===")

# Load counts
counts <- read.csv("counts_timepoint60_no_outlier.csv", row.names = 1, check.names = FALSE)

metadata <- read.csv("metv2_timepoint60_with_PC2_no_outlier.csv", row.names = 1, check.names = FALSE)
all(row.names(metadata) %in% colnames(counts))

# Verify PC2 is present
if (!"PC2" %in% colnames(metadata)) {
  stop("PC2 column not found in metadata! Please run the QC pipeline first to add PC2 coordinates.")
}

message(sprintf("PC2 range: %.3f to %.3f", min(metadata$PC2), max(metadata$PC2)))
message(sprintf("PC2 mean: %.3f, SD: %.3f", mean(metadata$PC2), sd(metadata$PC2)))
...
# Ensure PC2 is numeric (should already be, but just in case)
metadata$PC2 <- as.numeric(metadata$PC2)

# Center and scale PC2 for better model convergence
metadata$PC2 <- scale(metadata$PC2)[,1]
message("PC2 has been centered and scaled")

message("\n=== PC2 will be included as covariate in all models ===")
...
#-------------------------------------------------------
# 3. DESEQ2 ANALYSIS FUNCTION WITH LFC SHRINKAGE AND PC2
...
run_wald_test <- function(counts, metadata, design_formula,
  ...
  # Verify PC2 is in metadata for this subset
  if ("PC2" %in% colnames(metadata)) {
    message(sprintf(" PC2 range in subset: %.3f to %.3f", 
                    min(metadata$PC2), max(metadata$PC2)))
  }
  ...
  if (!is.null(contrast)) {
    ...
    # For interaction terms, use the last coefficient (excluding PC2)
    # Find the last non-PC2 coefficient
    non_pc2_coefs <- available_coefs[!grepl("^PC2$", available_coefs)]
    coef_name <- non_pc2_coefs[length(non_pc2_coefs)]
  ...
  res_df <- res_df %>%
    mutate(
      significant = pvalue < alpha,  # CHANGED: using pvalue instead of padj
      direction = case_when(
        log2FoldChange > 0 ~ "up",
      ...
      sig_category = case_when(
        is.na(pvalue) ~ "ns",
        pvalue == 0 ~ "***", 
        pvalue < 0.001 ~ "***",  # CHANGED: using pvalue thresholds
        pvalue < 0.01 ~ "**",
        pvalue < 0.05 ~ "*",
        pvalue < 0.1 ~ ".",
        TRUE ~ "ns"
    ...
    arrange(pvalue)  # CHANGED: sort by pvalue instead of padj
...
#--------------------------------------------------
# 4. MAIN ANALYSIS WORKFLOW (WITH PC2 COVARIATE)
#--------------------------------------------------

# Create output directory
output_dir <- "wald_test_results_rawPvalue_PC2_noShrink_no_outlier"
...
message("\n")
message("========================================================")
message("  ALL MODELS INCLUDE PC2 AS A COVARIATE")
message("  PC2 calculated from PCA on all timepoints")
message("  USING RAW P-VALUES (pvalue < 0.05) FOR ALL ANALYSES")
message("========================================================")
message("\n")

# ===============================================================
# a. MAIN EFFECTS ANALYSIS (with PC2)
# ===============================================================

message("\n==== MAIN EFFECTS ANALYSIS (with PC2 covariate) ====")

# Treatment effect (controlling for PC2)
full_results$treatment_main <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_Main_Effect_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Sex effect (controlling for PC2)
full_results$sex_main <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + sex,
  contrast = c("sex", "male", "female"),
  test_name = "Sex_Main_Effect_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Strain effect (controlling for PC2)
full_results$strain_main <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + strain,
  contrast = c("strain", "DBA_2J", "C57BL_6"),
  test_name = "Strain_Main_Effect_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# ===============================================================
# b. TWO-WAY INTERACTION ANALYSIS (with PC2)
# ===============================================================

message("\n==== TWO-WAY INTERACTIONS (with PC2 covariate) ====")

# Treatment × Sex (controlling for PC2)
full_results$treatment_sex_interaction <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + treatment * sex,
  contrast = NULL,  # Test interaction term
  test_name = "Treatment_x_Sex_Interaction_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment × Strain (controlling for PC2)
full_results$treatment_strain_interaction <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + treatment * strain,
  contrast = NULL,  # Test interaction term
  test_name = "Treatment_x_Strain_Interaction_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Sex × Strain (controlling for PC2)
full_results$sex_strain_interaction <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + sex * strain,
  contrast = NULL,  # Test interaction term
  test_name = "Sex_x_Strain_Interaction_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# ===============================================================
# c. THREE-WAY INTERACTION ANALYSIS (with PC2)
# ===============================================================

message("\n==== THREE-WAY INTERACTION (with PC2 covariate) ====")

full_results$three_way_interaction <- run_wald_test(
  counts, metadata,
  design_formula = ~ PC2 + treatment * sex * strain,
  contrast = NULL,  # Test three-way interaction term
  test_name = "Three_Way_Interaction_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# ===============================================================
# d. STRATIFIED ANALYSES (with PC2)
# ===============================================================

message("\n==== STRATIFIED ANALYSES (with PC2 covariate) ====")
message("\n==== SEX-BASED ANALYSES ====")

# Treatment effect in females (controlling for PC2)
...
full_results$treatment_effect_in_all_females <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_females_PC2adj_rawP",
  apply_shrinkage = FALSE
)

message("\n==== TREATMENT/STRAIN INTERACTION IN FEMALES ====")

# Treatment and Strain interaction in females (controlling for PC2)
full_results$treatment_strain_interaction_in_all_females <- run_wald_test(
  counts[, female_idx],
  metadata[female_idx, ],
  design_formula = ~ PC2 + treatment * strain,
  contrast = NULL,
  test_name = "Treatment_strain_interaction_in_females_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment effect in males (controlling for PC2)
...
full_results$treatment_effect_in_all_males <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_males_PC2adj_rawP",
  apply_shrinkage = FALSE
)

message("\n==== TREATMENT/STRAIN INTERACTION IN MALES ====")

# Treatment and Strain interaction in males (controlling for PC2)
full_results$treatment_strain_interaction_in_all_males <- run_wald_test(
  counts[, male_idx],
  metadata[male_idx, ],
  design_formula = ~ PC2 + treatment * strain,
  contrast = NULL,
  test_name = "Treatment_strain_interaction_in_males_PC2adj_rawP",
  apply_shrinkage = FALSE
)

message("\n==== STRAIN-BASED ANALYSES ====")

# Treatment effect in C57BL_6 (controlling for PC2)
...
full_results$treatment_effect_in_all_C57BL6 <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_C57BL6_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment/sex interaction in C57BL_6 (controlling for PC2)
full_results$treatment_sex_interaction_in_all_C57BL6 <- run_wald_test(
  counts[, c57_idx],
  metadata[c57_idx, ],
  design_formula = ~ PC2 + treatment * sex,
  contrast = NULL,
  test_name = "Treatment_sex_interaction_in_C57BL6_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment effect in DBA_2J (controlling for PC2)
...
full_results$treatment_effect_in_all_DBA2J <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_DBA2J_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment/sex interaction in DBA_2J (controlling for PC2)
full_results$treatment_sex_interaction_in_all_DBA2J <- run_wald_test(
  counts[, dba_idx],
  metadata[dba_idx, ],
  design_formula = ~ PC2 + treatment * sex,
  contrast = NULL,
  test_name = "Treatment_sex_interaction_in_DBA2J_PC2adj_rawP",
  apply_shrinkage = FALSE
)

message("\n==== TREATMENT EFFECT BY STRAIN & SEX ====")

# Treatment effect in C57BL_6 Females (controlling for PC2)
...
full_results$treatment_effect_in_female_C57BL6 <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_female_C57BL6_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment effect in DBA_2J Females (controlling for PC2)
...
full_results$treatment_effect_in_female_DBA2J <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_female_DBA2J_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment effect in C57BL_6 Males (controlling for PC2)
...
full_results$treatment_effect_in_male_C57BL6 <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_male_C57BL6_PC2adj_rawP",
  apply_shrinkage = FALSE
)

# Treatment effect in DBA_2J Males (controlling for PC2)
...
full_results$treatment_effect_in_male_DBA2J <- run_wald_test(
  ...
  design_formula = ~ PC2 + treatment,
  contrast = c("treatment", "THC", "vehicle"),
  test_name = "Treatment_in_male_DBA2J_PC2adj_rawP",
  apply_shrinkage = FALSE
)
...
create_interaction_pca <- function(dds, interaction_name, highlight_genes = NULL) {
  ...
  # Assemble data - FIXED TO AVOID DUPLICATE PC2 COLUMN
  pca_data <- as.data.frame(pca$x[, 1:4])
  pca_data$sample_name <- rownames(pca_data)
  
  # Get metadata and rename PC2 covariate to avoid conflict with PCA PC2
  metadata_cols <- as.data.frame(colData(vsd))
  if ("PC2" %in% colnames(metadata_cols)) {
    colnames(metadata_cols)[colnames(metadata_cols) == "PC2"] <- "PC2_covariate"
  }
  
  # Now safe to cbind without duplicate column names
...
for (name in names(full_results)) {
  sig_genes <- full_results[[name]]$results %>%
    filter(!is.na(pvalue) & pvalue <= 0.05) %>%  # CHANGED: using pvalue
    pull(ensembl_id)
...
if (length(combined_pca_plots) > 0) {
  ...
  combined_pca <- wrap_plots(combined_pca_plots, ncol = ncol_layout, nrow = nrow_layout) +
    plot_annotation(
      ...
      subtitle = "Based on significant genes (pvalue ≤ 0.05) from each model | PC2 included as covariate",
      theme = theme(
...
create_volcano_plot <- function(res_df, title, my_top = 10, 
                                lfc_threshold = 1, p_threshold = 0.05) {  # CHANGED parameter name
  
  total_genes <- nrow(res_df)
  
  volcano_data <- res_df %>%
    filter(!is.na(pvalue) & !is.na(log2FoldChange)) %>%  # CHANGED: using pvalue
    mutate(
      sig_status = case_when(
        pvalue < p_threshold & log2FoldChange > lfc_threshold ~ "Up",  # CHANGED
        pvalue < p_threshold & log2FoldChange < -lfc_threshold ~ "Down",  # CHANGED
        pvalue < p_threshold ~ "Sig_no_LFC",  # CHANGED
        TRUE ~ "NS"
      ),
      neg_log10_pvalue = pmin(-log10(pvalue + 1e-300), 300)  # CHANGED: using pvalue
    )
  ...
  top_genes <- volcano_data %>%
    ...
    arrange(pvalue) %>%  # CHANGED: sort by pvalue
    slice_head(n = my_top) %>%
    pull(gene_symbol)
  
  p <- ggplot(volcano_data, aes(x = log2FoldChange, y = neg_log10_pvalue)) +  # CHANGED y-axis
    ...
    geom_hline(yintercept = -log10(p_threshold),  # CHANGED
               linetype = "dashed", alpha = 0.5) +
    geom_text_repel(
    ...
    labs(
      x = "log2 Fold Change",
      y = "-log10(p-value)",  # CHANGED axis label
      title = title,
      subtitle = sprintf("Sig genes: %d up, %d down | %d/%d genes plotted (raw p < 0.05)",  # CHANGED
...
for (model_name in names(full_results)) {
  ...
  n_sig <- sum(res_df$pvalue < 0.05, na.rm = TRUE)  # CHANGED: using pvalue
  ...
  if (n_sig >= 5) {
    volcano_plot <- create_volcano_plot(
      ...
      my_top = 10,
      lfc_threshold = 1,
      p_threshold = 0.05  # CHANGED parameter name
    )
...
if (length(volcano_plots) > 1) {
  ...
  combined_volcano <- wrap_plots(volcano_plots, ncol = ncol_volcano, nrow = nrow_volcano) +
    plot_annotation(
      ...
      subtitle = "Top 10 DEGs labeled | Based on raw p-values (p < 0.05) | PC2 covariate adjusted",
      theme = theme(
...
create_deg_heatmap <- function(dds, res_df, title, my_top = 20) {
  ...
  # CHANGED: rank by pvalue instead of padj
  top_genes <- res_df %>%
    filter(!is.na(pvalue)) %>%
    arrange(pvalue) %>%
    slice_head(n = my_top) %>%
...
deg_summary <- data.frame(
  ...
  Up = sapply(full_results, function(x) {
    sum(x$results$pvalue < 0.05 & x$results$log2FoldChange > 0, na.rm = TRUE)  # CHANGED
  }),
...
deg_barplot <- ggplot(deg_summary, aes(x = Model, y = Count, fill = Direction)) +
  ...
  labs(
    ...
    subtitle = "Based on raw p-value < 0.05 | PC2 included as covariate",
    x = "Model",
...
# PC2 summary
pc2_summary <- data.frame(
  Metric = c("Mean", "SD", "Min", "Max", "Median"),
  Value = c(
    round(mean(metadata$PC2), 4),
    round(sd(metadata$PC2), 4),
    round(min(metadata$PC2), 4),
    round(max(metadata$PC2), 4),
    round(median(metadata$PC2), 4)
  )
)
...
write.csv(pc2_summary,
          file.path(output_dir, "PC2_covariate_summary.csv"),
          row.names = FALSE)
